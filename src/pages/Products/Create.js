...
function CreateProduct() {
  ...
  const handleSubmit = async (event) => {
    ...
    const embeddingsResponse = await axios.post(
      "http://192.168.29.18:3015/vector/generate-embedding",
      {
        namesapce: formData.productName,
      }
    );

    // Extract the embeddings from the response
    const { namesapceembedding } = embeddingsResponse.data;

    // Add the embeddings to the updated product data
    formData.nameEmbedding = namesapceembedding;
    //updatedData.descriptionEmbedding = descriptionEmbedding;
    console.log(formData);

    try {
      const response = await axios.post(
        "http://192.168.29.18:3015/products/create",
        formData
      );
